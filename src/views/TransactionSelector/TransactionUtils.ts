...
const parseTransactions = (rawTransactions, url: string, bank: Banks, account: Account): Transaction[] => {
  ...
  if (bank === Banks.ABN) {
    console.log(`${rawTransactions.mutationsList.mutations.length} transactions in response`);
  ...
  if (bank === Banks.BUNQ) {
    console.log(`${rawTransactions.Response.length} transactions in response`);
