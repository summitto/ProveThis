...
function detectToolVersion(toolName: string, toolBin: string, versionArg: string, transform: (arg: string) => string): Promise<SemVer> {
  ...
  console.log(`Command: ${toolBin} ${versionArg}`);
  ...
  return new Promise((resolve, reject) => {
    ...
    tool.stderr.on('data', (data: Buffer) => {
      reject(new Error(`Failed to detect ${toolName}.\nReason: ${data.toString('utf8')}`));
    });
...
async function detectEcdsaPyLib(pythonBin: string): Promise<void> {
  // special case for ecdsa detection script - it's executed right from the bundle instead of copying it to cache dir first
  const bundledStaticDir = resolvePath(__dirname, '../static');
  const ecdsaImportTestPath = join(bundledStaticDir, 'test_ecdsa.py');
  const ecdsaVersionTestPath = join(bundledStaticDir, 'test_ecdsa_version.py');

  /* eslint-disable max-len */
  const exists = await detectToolVersion('ecdsa python library existence', pythonBin, ecdsaImportTestPath, () => '0')
    .then(() => true)
    .catch((e) => {
      console.log('error detecting ecdsa:', e);
      return false;
    });

  if (!exists) {
    throw new Error(`ecdsa>=${ECDSA_PY_VERSION} python library was not found`);
  }

  let version: SemVer;
  try {
    version = await detectToolVersion('ecdsa python library', pythonBin, ecdsaVersionTestPath, (arg: string) => arg.replace('\n', ''));
  } catch (e) {
    // can't do much here if ecdsa is found but cannot detect the version
    console.log('failed to detect ecdsa py lib version, skipping');
    return;
  }

  console.log('Detected ecdsa py', version.version);
  const versionOk = versionGreaterOrEqual(version, ECDSA_PY_VERSION);
  if (!versionOk) {
    throw new Error(`ecdsa python library version requirement is not satisfied, minimum required is ${ECDSA_PY_VERSION}, found ${version?.version}`);
  }
...
async function testTools(conf: ToolsConfig) {
  ...
  await detectEcdsaPyLib(conf.python);
  /* eslint-enable max-len */
}
